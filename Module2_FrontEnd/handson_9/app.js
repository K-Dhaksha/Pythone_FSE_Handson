...
const totalCreditsVal = courses.reduce(
    (total, course) => total + course.credits, 0
);
console.log(totalCreditsVal);
...
function renderCourses(courseArray) {
    ...
    courseArray.forEach(course => {
        ...
        article.setAttribute("aria-label", `${course.name}, code ${course.code}, ${course.credits} credits. Press Enter to select.`);
    ...
    // Update live region textContent
    totalPara.textContent = `Total Credits: ${credits} | Courses Found: ${courseArray.length}`;
}
...
async function loadCourses() {
    grid.innerHTML = '<p style="text-align:center" role="status" aria-live="polite">Loading courses...</p>';
...
// Event Delegation for keyboard accessibility (Enter/Space triggers click)
...
// Mobile Hamburger Navigation & ARIA toggles
...
async function loadNotifications(url) {
    try {
        ...
        if (notificationsGrid) notificationsGrid.innerHTML = '';
        ...
        if (errorText) errorText.textContent = `Something went wrong: ${error.message}`;
